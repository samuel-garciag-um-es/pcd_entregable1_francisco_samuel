...
class Universidad:
    ...
    def añadir_asignatura(self,persona,asignatura):
        if isinstance(asignatura, Asignatura):
            if isinstance(persona, Profesor):
                persona.asignaturas.append(asignatura)
            elif isinstance(persona, Estudiante):
                persona.asignaturas.append(asignatura)
            else:
                raise TypeError("Persona tiene que ser de la clase Profesor o Estudiante")
        else:
            raise TypeError("Asignatura tiene que ser de la clase Asignatura")

    def eliminar_asignatura(self,persona,asignatura):
        if isinstance(asignatura, Asignatura):
            if isinstance(persona, Profesor):
                persona.asignaturas.remove(asignatura)
            elif isinstance(persona, Estudiante):
                persona.asignaturas.remove(asignatura)
            else:
                raise TypeError("Persona tiene que ser de la clase Profesor o Estudiante")
...
umu.mostrar_asignaturas_alumno(p1)
umu.eliminar_asignatura(p5, Asignatura("1", 23))
umu.eliminar_asignatura(p1, Asignatura("1", 23))
umu.añadir_asignatura(p5, Asignatura("mate", 234))
umu.añadir_asignatura(p1, Asignatura("lengua", 3245))
umu.mostrar_asignaturas_profesor(p5)
